...
const StoreDetails = (props) => {
  ...
  const [search, setSearch] = useState(false)
    ...
    useEffect(() => {
        fetch(`https://damp-caverns-66344.herokuapp.com/stores/${props.match.params.store_id}/categories`)
          .then(res => res.json())
          .then(categories => {
    ...
    useEffect(() => {
        fetch(`https://damp-caverns-66344.herokuapp.com/stores/${props.match.params.store_id}`)
          .then(res => res.json())
          .then(store => {
            setStore(store)
          })
...
const settings = {
  ...
  const onDetected = barcode => {
     console.log(barcode)
     fetch(`https://damp-caverns-66344.herokuapp.com/stores/${props.match.params.store_id}/offers/search/${barcode}`)
          .then(res => res.json())
          .then(offer => {
            console.log(offer)
            //setOffer(offer)
  ...
  const searchOffers = (e) => {
    ...
    e.preventDefault()
    let form = e.target
    fetch(`https://damp-caverns-66344.herokuapp.com/stores/${props.match.params.store_id}/search/${form[0].value}`)
      .then(res => res.json())
      .then(offers => {
        setOffers(offers)
        setSearch(true)
